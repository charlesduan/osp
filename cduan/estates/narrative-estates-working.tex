...
\end{itemize}
This is complete and unambiguous. When writing exam answers or legal documents,
it is highly advisable that you use sentences with these exact forms, filling
them in as if they were Mad Libs:
\begin{itemize}
\item \emph{[Person]} owns \emph{[land name]} in fee simple absolute.
\item \emph{[Person]} has a life estate in \emph{[land name]} for the life of
\emph{[person]}.
\item \emph{[Person]} has a \emph{[reversion/remainder]}, following the
\emph{[preceding interest]}, which will become \emph{[possessory estate]}.
\end{itemize}
For purposes of taking notes or working out problems on paper, though, the full
textual description can be too wordy.
We can omit ``Blackacre'' given that only
...
have? A complete conveyance simply transfers wholesale whatever interest the
conveyor has. So to effect a conveyance from A to C, all we need to do is to
copy A's property interest over to a new column for C:
\begin{center}
\begin{tabular}{l|lll}
Event & A & B & C \\
\hline
