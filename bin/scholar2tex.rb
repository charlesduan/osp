...
# encoding: utf-8
#
# Copyright 2014-2024 Charles Duan.
#
  ...
  def process_elt_a(elt)
    if elt['class'] == 'gsl_pagenum'
      ...
      if elt['href'] =~ /^#\[(.*)\]$/
        fnref = $1
        if @footnotes[fnref]
          "\\readingfootnote{#{fnref}}{#{@footnotes[fnref].strip}}"
        else
...
URI.open(ARGV[0]) do |f|
  ...
  puts "\\reading{#{cp.caption.gsub("\n", "\\\\\\\\\n")}}"
  puts "\\readingcite{#{cp.citation}}"
  puts "% \\court{#{cp.court}}"
  puts
  puts cp.text
end
